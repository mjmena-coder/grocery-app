...
def test_bulk_move_kitchen_staples_to_store(client, session):     
    """                                                           
    Asserts POST /grocery-list/staples/move-all moves all active kitchen staples                                                   
    into the designated target store list and unsets their is_kitchen_staple flag.                                           
    """                                                           
    staple1 = GroceryItem(                                        
        canonical_name="Olive Oil",                               
        quantity_display="1 bottle",                              
        category="PANTRY",                                        
        assigned_store="Unassigned",                              
        is_active=True,                                           
    ...
    session.add_all([staple1, staple2])                           
    session.commit()                                              
    ...
    # Verify state updates in DB                                  
    session.refresh(staple1)                                      
    session.refresh(staple2)                                      
                                                                  
    assert staple1.is_kitchen_staple is False                     
    assert staple1.assigned_store_override == "Whole Foods"       
    assert staple2.is_kitchen_staple is False                     
    assert staple2.assigned_store_override == "Whole Foods"       
...
def test_bulk_move_staples_when_none_exist(client, session):      
    """Asserts bulk moving staples when no staples exist completes
gracefully with moved_count=0."""                                 
    res = client.post("/grocery-list/staples/move-all?target_store=TraderJoe's")                                                           
    assert res.status_code == 200                                 
    assert res.json()["moved_count"] == 0